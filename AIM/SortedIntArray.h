...
public:
	...
	CExtendIntArray(CExtendIntArray *other): Count(other->Count), length(other->Count), support(other->support) { arr = new int[length]; memcpy(arr, other->arr, Count * sizeof(int)); }
...
{
public:
	CExtendingArray(): Count(0), length(BASIC_INT_ARRAY_LENGTH) { buffer = new T[BASIC_INT_ARRAY_LENGTH](); }
	CExtendingArray(CExtendIntArray *other): Count(other->Count), length(other->Count) { arr = new T[length]; memcpy(buffer, other->buffer, Count * sizeof(T)); }
